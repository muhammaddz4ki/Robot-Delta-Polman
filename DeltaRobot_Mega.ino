...
void runAutoSequence() {
  ...
  if (!moveToXYZ(0, 0, -220))                                      { autoRunRunning = false; return; }
...
void runAutoSequence1() {
  ...
  if (!moveToXYZ(0, 0, -200))                                      { autoRunRunning = false; return; }
...
void setup() {
  ...
  digitalWrite(relayPin, RELAY_OFF_STATE); // Default Dinamo Hisap MATI (OFF)
